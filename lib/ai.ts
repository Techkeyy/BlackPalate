...
export async function draftCampaignWithAI(input: CampaignDraftPrompt): Promise<GeneratedCampaignDraft> {
...
10. feedbackQuestions: Array of 3-4 structured research questions [{ id: "q1", prompt: "...", type: "rating" | "text" | "choice", options?: ["..."] }]`;
        ...
        if (content) {
          ...
          return {
            ...
            feedbackQuestions: parsed.feedbackQuestions || [
              { id: 'q1', prompt: 'Rate flavor balance and seasoning accuracy:', type: 'rating' },
              { id: 'q2', prompt: 'Rate dish presentation and plating appeal:', type: 'rating' },
              { id: 'q3', prompt: 'Specific feedback on texture, temperature, and cooking technique:', type: 'text' },
            ],
          };
        }
      }
    } catch (err) {
      console.warn('[AI] Error calling LLM provider, falling back to deterministic template:', err);
    }
  }

  // Resilient heuristic draft generator
  return {
...
): Promise<Omit<SynthesisReport, 'id' | 'generatedAt'>> {
...
3. cohortTrends: Array of [{ cohort: "e.g. Cuisine Enthusiasts", sentiment: "e.g. Highly positive", takeaways: "..." }]
4. recommendations: Array of 3-4 specific, actionable recipe or menu adjustments`,
      ...
      console.warn('[AI] Synthesis LLM call failed, using statistical synthesis:', err);
    }
  }

  // Statistical heuristic synthesis
