...
async function runAudit() {
  ...
  console.log('\n--- Phase 8: Directive 002F Secret Incident Remediation & Cross-Workspace Security ---');

  // Check 1: Secret Hygiene (generic properties — never embed a real secret value)
  const fs = await import('node:fs');
  const authFileContent = fs.readFileSync('./lib/auth/neon-server.ts', 'utf8');

  test('Secret Remediation: No hardcoded auth-secret fallback in source', () => {
    assert.strictEqual(authFileContent.includes('process.env.NEON_AUTH_COOKIE_SECRET ||'), false);
    const hardcodedSecretAssign = /COOKIE_SECRET['"]?\s*[:=]\s*['"][0-9a-fA-F]{32,}['"]/;
    assert.strictEqual(hardcodedSecretAssign.test(authFileContent), false);
    assert.ok(authFileContent.includes('process.env.NEON_AUTH_COOKIE_SECRET'));
  });

  test('Fail-Closed Auth: Missing NEON_AUTH_COOKIE_SECRET throws configuration error', () => {
    let threw = false;
    try {
      ...
      threw = true;
      assert.match(e.message, /NEON_AUTH_CONFIGURATION_ERROR/);
    }
    assert.strictEqual(threw, true);
  });

  // Check 2: Cross-Workspace Mutex
  const operatorA = await db.createUser({
    displayName: 'Chef Alice',
    email: 'alice@restaurant-a.audit',
    restaurantAuthUserId: 'neon_auth_alice',
  });

  const operatorB = await db.createUser({
    displayName: 'Chef Bob',
    email: 'bob@restaurant-b.audit',
    restaurantAuthUserId: 'neon_auth_bob',
  });

  const restA = await db.createRestaurant({
    name: 'Restaurant A',
    cuisine: ['French'],
  });

  const restB = await db.createRestaurant({
    name: 'Restaurant B',
    cuisine: ['Japanese'],
  });

  await db.createMembership({ userId: operatorA.id, restaurantId: restA.id, role: 'OWNER' });
  await db.createMembership({ userId: operatorB.id, restaurantId: restB.id, role: 'OWNER' });

  await asyncTest('Cross-Workspace Isolation: Operator A can mutate A but is forbidden on B (403)', async () => {
    const memA_A = await db.getMembership(operatorA.id, restA.id);
    const memA_B = await db.getMembership(operatorA.id, restB.id);
    assert.strictEqual(Boolean(memA_A && (memA_A.role === 'OWNER' || memA_A.role === 'MANAGER')), true);
    assert.strictEqual(memA_B, null, 'Operator A must receive 403 FORBIDDEN_WORKSPACE on Restaurant B');
  });

  await asyncTest('Cross-Workspace Isolation: Operator B can mutate B but is forbidden on A (403)', async () => {
    const memB_B = await db.getMembership(operatorB.id, restB.id);
    const memB_A = await db.getMembership(operatorB.id, restA.id);
    assert.strictEqual(Boolean(memB_B && (memB_B.role === 'OWNER' || memB_B.role === 'MANAGER')), true);
    assert.strictEqual(memB_A, null, 'Operator B must receive 403 FORBIDDEN_WORKSPACE on Restaurant A');
  });

  // Check 3: Reward Integrity (No Fake TX Hash)
  await asyncTest('Reward Integrity: Unconfirmed/pending reward has txHash=null and never fake tx identifier', async () => {
    const receipt = await db.createRewardReceipt({
      applicationId: 'app_audit_clean_01',
      campaignId: restA.id,
      userId: operatorA.id,
      amountFly: '10',
      amountFlyWei: '10000000000000000000',
      idempotencyKey: 'blackpalate:reward:app_audit_clean_01',
      status: 'PENDING',
      txHash: null,
    });
    assert.strictEqual(receipt.txHash, null);
    assert.notStrictEqual(receipt.txHash, 'tx_flynet_confirmed');
  });
