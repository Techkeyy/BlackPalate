...
async function runIntegrationTests() {
  // Directive 002G: integration tests MUST run against an isolated TEST database.
  // They must NEVER write to production. Missing TEST_DATABASE_URL => SKIP.
  // TEST_DATABASE_URL === DATABASE_URL => hard REFUSE.
  const testDbUrl = process.env.TEST_DATABASE_URL;
  const prodDbUrl = process.env.DATABASE_URL || process.env.POSTGRES_URL;
  if (!testDbUrl) {
    console.log('[SKIP] Integration tests require TEST_DATABASE_URL pointing at an isolated Neon test branch.');
    console.log('       Create a Neon test branch/database and set TEST_DATABASE_URL. Production is never touched.');
    return;
  }
  if (prodDbUrl && testDbUrl === prodDbUrl) {
    console.error('[REFUSE] TEST_DATABASE_URL must not equal DATABASE_URL. Refusing to run against production.');
    process.exit(1);
  }
